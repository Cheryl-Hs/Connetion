...
//int rowsI=rs.getRow();
    ...
    public String getDate(HttpServletRequest request) throws SQLException {
        int total = MySqlHelper.getCount("select count(*) from `localserviceinfo_2`");//假设从数据库中取出100条
       // int total =MySqlHelper.getCount("select * from localserviceinfo_2"); //假设从数据库中取出100条
        ...
        int start=0;
        start=(page-1)*rows;
        String sqlString = "SELECT `id`,`label`,`name`,`phonecode`,`postcode`,`address`,`province`,`city`,`county`,`township`,`village`,`refobject`,`content`,`refrelation`,`url`,`pagekeyword`,`pageabstract`,`date`,`stat` FROM `localserviceinfo_2` limit "+start+" ,"+rows;//
