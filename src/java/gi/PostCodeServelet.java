 ...
 * @author lyunfan
 */
public class PostCodeServelet extends HttpServlet {
    ...
    public void submit(HttpServletRequest request) {
        ...
        if (deleted != null) {
            ...
            for (Object o : arr) {
                ...
                sqldeleteString = JsonToDBUtil.JODeleteDb("district_code_phone_post");
                Iterator itt = a.keys();
                int i = 0;
                while (itt.hasNext()) {
        ...
        if (inserted != null) {
            ...
            for (Object o : arr) {
                ...
                Object[] objects = new Object[a.size()];
                sqlInsertString = JsonToDBUtil.JOInsertDb(a, "district_code_phone_post",5);
                /* if(objects[a.size()-3]==""){
        ...
        if (updated != null) {
            ...
            for (Object o : arr) {
                ...
                Object[] objects = new Object[a.size()];
                String s=" SET `province`=?,`city`=?,`county`=?,`code`=?,`phone`=? WHERE `id`=?";
                sqlUpdateString = JsonToDBUtil.JOUpdateDb("district_code_phone_post",s);
                Iterator itt = a.keys();
                //  params=new Object[jo.size()];
                int i = 0;
               int count=6;
...
//int rowsI=rs.getRow();
    ...
    public String getDate(HttpServletRequest request) throws SQLException {
        int total = MySqlHelper.getCount("select count(*) from `district_code_phone_post`");//假设从数据库中取出100条
       // int total =MySqlHelper.getCount("select * from localserviceinfo_2"); //假设从数据库中取出100条
        ...
        int start=0;
        start=(page-1)*rows;
        String sqlString = "SELECT * FROM `district_code_phone_post` limit "+start+" ,"+rows;//
