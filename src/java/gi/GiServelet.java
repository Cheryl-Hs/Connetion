 ...
 * @author lyunfan
 */
public class GiServelet extends HttpServlet {
    ...
    public void submit(HttpServletRequest request) {
        ...
        if (deleted != null) {
            ...
            for (Object o : arr) {
                ...
                sqldeleteString = JsonToDBUtil.JODeleteDb("localserviceinfo");
                Iterator itt = a.keys();
                int i = 0;
                while (itt.hasNext()) {
        ...
        if (inserted != null) {
            ...
            for (Object o : arr) {
                ...
                Object[] objects = new Object[a.size()];
                sqlInsertString = JsonToDBUtil.JOInsertDb(a, "localserviceinfo",19);
                /* if(objects[a.size()-3]==""){
        ...
        if (updated != null) {
            ...
            for (Object o : arr) {
                ...
                Object[] objects = new Object[a.size()-4];
                String s=" SET `name`=?,`label`=?,`address`=?,`postcode`=?,`phonecode`=?,`phone`=?,`province`=?,`city`=?,`county`=?,`township`=?,`village`=?,`road`=?,`roadNo`=?,`building`=?,`floor`=?,`room`=?,`date`=?,`url`=?,`flag`=? WHERE `id`=?";
                sqlUpdateString = JsonToDBUtil.JOUpdateDb("localserviceinfo",s);
                Iterator itt = a.keys();
                //  params=new Object[jo.size()];
                int i = 0;
                int count=20;
...
//int rowsI=rs.getRow();
    ...
    public String getDate(HttpServletRequest request) throws SQLException {
       ...
       // int total =MySqlHelper.getCount("select * from localserviceinfo_2"); //假设从数据库中取出100条
        ...
        int start=0;
        start=(page-1)*rows;
        String sqlString = "SELECT * FROM `localserviceinfo` limit "+start+" ,"+rows;//
